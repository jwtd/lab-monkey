...
"""
Tests ParallelFPGAAdapterTests
"""
...
class MockAdapterTests(TestCase):
    """Tests a Package session via a Mock Adapter."""

    FOUND = None
    
    def run(self, result=None):
        """Only run the adapter tests when the adapter is detected"""
        if MockAdapterTests.FOUND == True or MockAdapter.detect():
            super(MockAdapterTests, self).run(result)
            MockAdapterTests.FOUND = True
        else:
            MockAdapterTests.FOUND = False
            #print 'MockAdapter not detected.'

    def setUp(self):
        """Loading Package from text file"""
        self.dut = Package.from_txt_file(exepath('../../tests/mocks/DES_65nm_Fuji.txt'))
