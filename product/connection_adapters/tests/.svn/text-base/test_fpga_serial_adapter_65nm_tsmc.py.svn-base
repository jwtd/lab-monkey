...
class FPGASerialAdapterTests(TestCase):
    """Tests a Package session via a FPGA Serial Adapter."""

    FOUND = None
    
    def run(self, result=None):
        """Only run the adapter tests when the adapter is detected"""
        if FPGASerialAdapterTests.FOUND == True or FPGASerialAdapter.detect():
            super(FPGASerialAdapterTests, self).run(result)
            FPGASerialAdapterTests.FOUND = True
        else:
            FPGASerialAdapterTests.FOUND = False
            #print 'FPGASerialAdapter not detected.'
            
    def setUp(self):
        """Loading Package from text file"""
        self.dut = Package.from_txt_file(exepath('../../repository/DES_65nm_TSMC.txt'), connect = 'Serial FPGA')
        # These caused sent to be populated which broke some of the tests
        #self.dut.big.cb.vco_cal_en.disable()
        #self.dut.big.cb.vco_ref_code.disable()
    ...
    def test_constants_assignment(self):
        ...
        # Verify default
        self.assertEqual(self.dut.big.lane_1['BIST_MODE'].check(), 1)
        
        # Verify all constants
        for constant in bist_mode_defaults:
            self.dut.big.lane_1.bist_mode.prepare(constant)
            self.assertEqual(self.dut.big.lane_1['BIST_MODE'].check(), bin_to_int(bist_mode_defaults[constant][1:]))
    ...
    def test_prepare_check_commit_at_register(self):
        ...
        self.assertEqual(self.dut.big.lane_1['BIST_MODE'].check(), 1)
        self.dut.big.lane_1.bist_mode.prepare('b1010')        
        self.assertEqual(self.dut.big.lane_1['BIST_MODE'].check(), 10)
        
        # Commit and check sent at register
        self.assertEqual(self.dut.big.lane_1.bist_mode.is_sent, False)
        self.assertEqual(self.dut.big.lane_1.bist_mode.sent, None)
        self.dut.big.lane_1.bist_mode.commit()        
        self.assertEqual(self.dut.big.lane_1.bist_mode.is_sent, True)
        self.assertEqual(self.dut.big.lane_1.bist_mode.sent, 10)

    def test_prepare_check_commit_at_block(self):
        """Prepare check and commit at block"""
        
        # Prepare and check at block
        self.assertEqual(self.dut.big.lane_1.check('BIST_MODE'), 1)
        self.dut.big.lane_1.bist_mode.prepare('b1010')        
        self.assertEqual(self.dut.big.lane_1.check('BIST_MODE'), 10)

        # Verify uniqueness
        results = self.dut.big.check('BIST_MODE')
        for lane in self.dut.big.lanes:
            label = self.dut.big.lanes[lane].label
            if label == 'lane_1':
    ...
    def test_prepare_check_commit_at_scr(self):
        ...
        results = self.dut.big.check('BIST_MODE')
        for lane in self.dut.big.lanes:
            self.assertEqual(results[self.dut.big.lanes[lane].label], 1)
        
        # Prepare
        self.dut.big.prepare('BIST_MODE', 'b1010')
        
        # Verify that it is now prepared
        results = self.dut.big.check('BIST_MODE')
        for lane in self.dut.big.lanes:
            self.assertEqual(results[self.dut.big.lanes[lane].label], 10)
        
        # Verify that none of the values have been sent
        for lane in self.dut.big.lanes:
            self.assertEqual(self.dut.big.lanes[lane].bist_mode.is_sent, False)
            self.assertEqual(self.dut.big.lanes[lane].bist_mode.sent, None)
        ...
        for lane in self.dut.big.lanes:
            self.assertEqual(self.dut.big.lanes[lane].bist_mode.is_sent, True)
            self.assertEqual(self.dut.big.lanes[lane].bist_mode.sent, 10)
    ...
    def test_prepare_and_clear(self):
        ...
        self.assertEqual(self.dut.big.lane_1.pending, [])
        
        # Prepare three registers
        self.dut.big.lane_1.tx_rclk_en.prepare('1')      # meta reference
        self.dut.big.lane_1['BIST_MODE'].prepare('b1010') # bracket reference
        self.dut.big.lane_1['SJ_FREQ'].prepared = 'b111'  # Value assignement
        
        # Get results and verify length and contents
        now_pending = self.dut.big.lane_1.pending
        self.assertEqual(len(now_pending), 3)
        self.assertEqual(('big.lane_1.tx_rclk_en : 0 => 1' in now_pending), True)
        self.assertEqual(('big.lane_1.bist_mode : 1 => 10' in now_pending), True)
        self.assertEqual(('big.lane_1.sj_freq : 0 => 7' in now_pending), True)
        
        # Selectively clear and verify
        self.dut.big.lane_1['BIST_MODE'].clear()
        now_pending = self.dut.big.lane_1.pending

        self.assertEqual(len(now_pending), 2)
        self.assertEqual(('big.lane_1.bist_mode : 0001 => 1010' in now_pending), False)
        
        # Clear and verify
        self.dut.big.lane_1.tx_rclk_en.clear()
        self.dut.big.lane_1['SJ_FREQ'].clear()
        now_pending = self.dut.big.lane_1.pending
    ...
    def test_prepare_at_block(self):
        ...
        self.dut.big.lane_1.prepare('TX_RCLK_EN', 'b1')
        self.dut.big.lane_1.prepare('BIST_MODE', 'b1010')
        self.dut.big.lane_7.prepare('TX_RCLK_EN', 'b1')
        self.dut.big.lane_7.prepare('BIST_MODE', 'b1010')
        ...
        now_pending = self.dut.big.lane_7.pending
        self.assertEqual(len(now_pending), 2)
        self.assertEqual(('big.lane_7.tx_rclk_en : 0 => 1' in now_pending), True)
        self.assertEqual(('big.lane_7.bist_mode : 1 => 10' in now_pending), True)

        # Pending at the SCR should show both
        now_pending = self.dut.big.pending
        self.assertEqual(len(now_pending), 4)

        # Selectively clear and verify
        self.dut.big.lane_1.clear()
        now_pending = self.dut.big.lane_1.pending
        self.assertEqual(len(now_pending), 0)
    ...
    def test_prepare_at_scr(self):
        ...
        self.dut.big.prepare('TX_RCLK_EN', '1')
        ...
        self.assertEqual(('big.lane_0.tx_rclk_en : 0 => 1' in now_pending), True)
        self.assertEqual(('big.lane_1.tx_rclk_en : 0 => 1' in now_pending), True)
        self.assertEqual(('big.lane_2.tx_rclk_en : 0 => 1' in now_pending), True)
        self.assertEqual(('big.lane_7.tx_rclk_en : 0 => 1' in now_pending), True)
    ...
    def test_prepare_at_package(self):
        ...
        self.assertEqual(len(now_pending), 16)
        self.assertEqual(('big.lane_0.tx_rclk_en : 0 => 1' in now_pending), True)
        self.assertEqual(('big.lane_0.bist_mode : 1 => 10' in now_pending), True)
        self.assertEqual(('big.lane_7.tx_rclk_en : 0 => 1' in now_pending), True)
        self.assertEqual(('big.lane_7.bist_mode : 1 => 10' in now_pending), True)
    ...
    def test_autoenable(self):
        ...
        self.assertEqual(self.dut.big.autoenable, True)
        ...
        r = self.dut.big.common_block['VCO_CODE']
        ...
        # Should enable the bit because big's autoeanble is on
        r = self.dut.big.common_block['VCO_CODE']
        r.disable()
        self.assertEqual(r.enabled, False)
        r.set('b11111')
    ...
    def test_set_and_reset_at_register(self):
        ...
        # Set the value
        self.dut.big.lane_1.bist_mode.set('b1010')
        ...
        # Reset the value
        self.dut.big.lane_1.bist_mode.reset()

        # Verify that the value is now back at it's default
        self.assertEqual(self.dut.big.lane_1.bist_mode.is_sent, True)
        self.assertEqual(self.dut.big.lane_1.bist_mode.sent, 1)
    ...
    def test_set_and_reset_at_block(self):
        ...
        # Set the value
        self.dut.big.lane_1.set('BIST_MODE', 'b1010')
        ...
        for lane in self.dut.big.lanes:
            ...
            if label == 'lane_1':
                ...
                self.assertEqual(self.dut.big.lanes[lane].bist_mode.sent, 1)
    ...
    def test_set_and_reset_at_scr(self):
        ...
        # Set the value
        self.dut.big.set('BIST_MODE', 'b1010')
        
        # Verify that it has been set in all blocks
        for lane in self.dut.big.lanes:
            self.assertEqual(self.dut.big.lanes[lane].bist_mode.is_sent, True)
            self.assertEqual(self.dut.big.lanes[lane].bist_mode.sent, 10) #1010
    ...
    def test_refresh_inspect_at_register(self):
        ...
        # Set a value
        self.dut.big.lane_1.bist_mode.set(10)

        # Verify value with inspect
        self.assertEqual(self.dut.big.lane_1.bist_mode.inspect(), 1)
        
        # Refresh the output buffer
        self.dut.big.lane_1.bist_mode.refresh()
        
        # Verify value with inspect
        self.assertEqual(self.dut.big.lane_1.bist_mode.inspect(), 10)
    ...
    def test_get_inspect_at_register(self):
        ...
        # Refresh the output buffer
        self.assertEqual(self.dut.big.lane_1.bist_mode.get(), 10)
    ...
    def test_refresh_inspect_at_block(self):
        ...
        # Set a value
        self.dut.big.lane_1.set('BIST_MODE',10)
        
        # Verify value with inspect
        self.assertEqual(self.dut.big.lane_1.inspect('BIST_MODE'), 1)
        
        # Refresh the output buffer
        self.dut.big.lane_1.refresh()
        
        # Verify value with inspect
        self.assertEqual(self.dut.big.lane_1.inspect('BIST_MODE'), 10)
    ...
    def test_get_inspect_at_block(self):
        ...
        # Refresh the output buffer
        self.assertEqual(self.dut.big.lane_1.get('BIST_MODE'), 10)
    ...
    def test_refresh_inspect_at_scr(self):
        ...
        # Set a value
        self.dut.big.set('BIST_MODE',10)
        
        # Verify value with inspect
        results = self.dut.big.inspect('BIST_MODE')
        for lane in self.dut.big.lanes:
            self.assertEqual(results[self.dut.big.lanes[lane].label], 1)

        # Refresh the output buffer
        self.dut.big.refresh()
    ...
    def test_get_inspect_at_scr(self):
        ...
        # Refresh the output buffer
        results = self.dut.big.get('BIST_MODE')
        for lane in self.dut.big.lanes:
            self.assertEqual(results[self.dut.big.lanes[lane].label], 10)
