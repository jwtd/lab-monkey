...
class USBAdapterTests(TestCase):
    """Tests a Package session via a USB Adapter."""

    FOUND = None
    
    def run(self, result=None):
        """Only run the adapter tests when the adapter is detected"""
        if USBAdapterTests.FOUND == True or USBAdapter.detect():
            super(USBAdapterTests, self).run(result)
            USBAdapterTests.FOUND = True
        else:
            USBAdapterTests.FOUND = False
            #print 'USBAdapter not detected.'

    def setUp(self):
        """Loading Package from text file"""
        self.dut = Package.from_txt_file(exepath('../../tests/mocks/DES_65nm_Fuji.txt'), connect=False)
        #self.dut = Package.from_txt_file(exepath('../../tests/mocks/DES_90nm_TSMC_GT.txt'), connect=False)
        self.dut.connect('USB')
